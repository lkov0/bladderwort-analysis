...
UCSC=$HOME/Software/Aligners/UCSC_utils
...
# python3 1a_CutGenomesBySize.py -i $target_genome -o $outprefix --rename $tname --min 1000 --winsize $target_size #--debug
# # 
# # # # Split query genome by segments
# outprefix=$querydir/${qname}
# python3 1a_CutGenomesBySize.py -i $query_genome -o $outprefix --rename $qname --min 1000 --winsize $query_size #--debug
# 
# 
...
#       echo "lastz $target_win $query_win --ambiguous=iupac --format=axt --inner=2000 --xdrop=9400 --gappedthresh=3000 --hspthresh=2200 > $axtfile" >> $lastz_commands
#       echo "$UCSC/axtChain -linearGap=loose $axtfile -faT $target_win -faQ $query_win $chainfile" >> $chain_commands
#       
#       i=$(($i+1))
#       
#     done
