...
faSize $chaindir/1e_$tname.combined.fa -detailed > $target_sizes
rm $chaindir/1e_$tname.combined.fa

echo ------------ Done with target chr sizes --------------

cat $querydir/*.fa > $chaindir/1e_$qname.combined.fa
faSize $chaindir/1e_$qname.combined.fa -detailed > $query_sizes
...
chainMergeSort -inputList=$chaindir/1e_${tname}_${qname}_all.chains.txt > $chaindir/1e_${tname}_${qname}_all.chain.merge
chainPreNet $chaindir/1e_${tname}_${qname}_all.chain.merge $target_sizes $query_sizes $chaindir/1e_${tname}_${qname}_all.chain.merge.pre

echo ---------- Done with sort and filter chains ----------------

# ## Net the chains ##
chainNet $chaindir/1e_${tname}_${qname}_all.chain.merge.pre -minSpace=1 $target_sizes $query_sizes $chaindir/1f_${tname}_${qname}.target_$tname.net $chaindir/1f_${tname}_${qname}.query_$qname.net

echo ---------- Done with netting of chains ----------------


# ## Add synteny info to nets ##
netSyntenic $chaindir/1f_${tname}_${qname}.target_$tname.net $chaindir/1f_${tname}_${qname}.target_$tname.noClass.net


# Make 2bit files
faToTwoBit $targetdir/${tname}*.fa $chaindir/1g_$tname.seqs.2bit
faToTwoBit $querydir/${qname}*.fa $chaindir/1g_$qname.seqs.2bit

echo ---------- Done with 2bit file making ----------------

# Convert to MAF files
netToAxt $chaindir/1f_${tname}_${qname}.target_$tname.noClass.net $chaindir/1e_${tname}_${qname}_all.chain.merge.pre $chaindir/1g_$tname.seqs.2bit $chaindir/1g_$qname.seqs.2bit \
  $chaindir/1h_${tname}_${qname}.multialign.axt
axtSort $chaindir/1h_${tname}_${qname}.multialign.axt $chaindir/1h_${tname}_${qname}.multialign.sorted.axt
axtToMaf $chaindir/1h_${tname}_${qname}.multialign.sorted.axt $target_sizes $query_sizes $chaindir/1h_${tname}_${qname}.multialign.sorted.maf
