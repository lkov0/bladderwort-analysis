...
# for sample in 1L 2L 3L 1R 2R 3R 1B 2B 3B 1S 2S 3S; do cat ${sample}*.trimmed.fq.gz > ${sample}.combined.trimmed.fq.gz; done
...
#RScript to extract matrix of normalized counts

#RScript to append 3' data to gene pairs file


#make gff
# cat u.gibba_NEW.gffheader.txt u.gibba_NEW.genes_for_3prime.txt > u.gibba_NEW.genes_for_3prime.gff
#star transcript quantification doesn't seem to be working
...
#     macs2 callpeak -t $alignmentDir/${seq}_ugibba_lkAligned.sortedByCoord.out.bam -g 90e6 -n $seq --outdir $macDir
...
#     featureCounts -t gene -g ID -a $myAnnos -o $fcDir/${seq}_featureCounts.txt $alignmentDir/${seq}_ugibba_lkAligned.sortedByCoord.out.bam
done
